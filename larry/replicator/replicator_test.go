...
func createReplicator(t *testing.T, policy Policy, home, srcType, dstType string, tables []string) (larry.Database, larry.Database) {
	...
	case "mongo":
		...
		cfg := mongo.DefaultMongoConfig(homeDestination, tables)
		cfg.DropTables = true
		dbs, err := mongo.NewMongoDB(cfg)
		if err != nil {
			t.Fatal(err)
		}
		dbDestination = dbs
	default:
		...
		dbs, err := level.NewLevelDB(level.DefaultLevelConfig(homeDestination, tables))
		if err != nil {
			t.Fatal(err)
		}
		dbDestination = dbs
	...
	default:
		...
		dbs, err := level.NewLevelDB(level.DefaultLevelConfig(homeSource, tables))
		if err != nil {
			panic(err)
		}
		dbSource = dbs
