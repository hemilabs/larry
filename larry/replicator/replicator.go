...
func NewReplicatorDB(cfg *ReplicatorConfig, source, sink larry.Database) (larry.Database, error) {
	if cfg == nil {
	...
	var err error
	bdb.jdb, err = level.NewLevelDB(level.DefaultLevelConfig(cfg.Home, []string{""}))
	if err != nil {
		return nil, err
	}
	return bdb, nil
