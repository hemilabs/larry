...
var (
	_ larry.Batch       = (*replicatorBatch)(nil)
	_ larry.Database    = (*replicatorDB)(nil)
	_ larry.Iterator    = (*replicatorIterator)(nil)
	_ larry.Range       = (*replicatorRange)(nil)
	_ larry.Transaction = (*replicatorTX)(nil)
...
func newJournalID(ctx context.Context, db larry.Database) (uint64, error) {
	var newID uint64
	return newID, db.Update(ctx, func(ctx context.Context, tx larry.Transaction) error {
...
type replicatorDB struct {
	...
	source larry.Database // source
	sink   larry.Database // sink
	jdb    larry.Database // journal, for now only support leveldb
...
func DefaultReplicatorConfig(home string, policy Policy) *Config {
	return &Config{
		Home:   home,
		Policy: policy,
	}
...
func NewReplicatorDB(cfg *Config, source, sink larry.Database) (larry.Database, error) {
	if cfg == nil {
		return nil, larry.ErrInvalidConfig
	}
	if cfg.Home == "" {
		return nil, errors.New("must provide journal home")
	}
	bdb := &replicatorDB{
		cfg:    cfg,
		source: source,
	...
	var err error
	bdb.jdb, err = leveldb.NewLevelDB(leveldb.DefaultLevelDBConfig(cfg.Home, []string{""}))
	if err != nil {
		return nil, err
	}
	return bdb, nil
...
func (b *replicatorDB) replayJournal(ctx context.Context, key []byte, value []byte) error {
	...
	// Commit batch
	err = b.sink.Update(ctx, func(ctx context.Context, tx larry.Transaction) error {
...
func (b *replicatorDB) Del(ctx context.Context, table string, key []byte) error {
	return b.directAccess(ctx, func(ctx context.Context) (*journal, error) {
		err := b.source.Update(ctx, func(ctx context.Context, tx larry.Transaction) error {
		...
		j.ops.PushFront(&larry.Operation{
			Op:    larry.OpDel,
			Table: table,
			Key:   key,
		})
...
func (b *replicatorDB) Has(ctx context.Context, table string, key []byte) (bool, error) {
	if b.closed(ctx) {
		return false, larry.ErrDBClosed
	}
...
func (b *replicatorDB) Begin(ctx context.Context, write bool) (larry.Transaction, error) {
...
func (b *replicatorDB) execute(ctx context.Context, write bool, callback func(ctx context.Context, tx larry.Transaction) error) error {
...
func (b *replicatorDB) View(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return b.execute(ctx, false, callback)
}

func (b *replicatorDB) Update(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return b.execute(ctx, true, callback)
}

func (b *replicatorDB) NewIterator(ctx context.Context, table string) (larry.Iterator, error) {
...
func (b *replicatorDB) NewRange(ctx context.Context, table string, start, end []byte) (larry.Range, error) {
...
func (b *replicatorDB) NewBatch(ctx context.Context) (larry.Batch, error) {
...
func (tx *replicatorTX) Write(ctx context.Context, b larry.Batch) error {
