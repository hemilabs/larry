...
var log = loggo.GetLogger("bitcask")
...
var (
	_ larry.Batch       = (*bitcaskBatch)(nil)
	_ larry.Database    = (*bitcaskDB)(nil)
	_ larry.Iterator    = (*bitcaskIterator)(nil)
	_ larry.Range       = (*bitcaskRange)(nil)
	_ larry.Transaction = (*bitcaskTX)(nil)
)
...
func NewBitcaskDB(cfg *BitcaskConfig) (larry.Database, error) {
	if cfg == nil {
...
func (b *bitcaskDB) k2b(table string, key []byte) bitcask.Key {
	return bitcask.Key(larry.NewCompositeKey(table, key))
}
...
func (b *bitcaskDB) Begin(_ context.Context, write bool) (larry.Transaction, error) {
	if write {
		b.txMtx.Lock()
	}
	tx := b.db.Transaction()
...
func (b *bitcaskDB) execute(ctx context.Context, write bool, callback func(ctx context.Context, tx larry.Transaction) error) error {
...
func (b *bitcaskDB) View(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return b.execute(ctx, false, callback)
}

func (b *bitcaskDB) Update(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return b.execute(ctx, true, callback)
}

func (b *bitcaskDB) NewIterator(ctx context.Context, table string) (larry.Iterator, error) {
	if _, ok := b.tables[table]; !ok {
		return nil, larry.ErrTableNotFound
	}
...
func (b *bitcaskDB) NewRange(ctx context.Context, table string, start, end []byte) (larry.Range, error) {
	if _, ok := b.tables[table]; !ok {
		return nil, larry.ErrTableNotFound
	}
...
func (b *bitcaskDB) NewBatch(ctx context.Context) (larry.Batch, error) {
...
func (tx *bitcaskTX) Del(ctx context.Context, table string, key []byte) error {
	...
	return tx.tx.Delete(tx.db.k2b(table, key))
}
...
func (tx *bitcaskTX) Put(ctx context.Context, table string, key []byte, value []byte) error {
	...
	return tx.tx.Put(tx.db.k2b(table, key), value)
}
...
func (tx *bitcaskTX) Write(ctx context.Context, b larry.Batch) error {
...
func (bi *bitcaskIterator) Key(_ context.Context) []byte {
	if bi.cur != nil && bi.cur.Key() != nil {
		return larry.KeyFromComposite(bi.table, bi.cur.Key())
	}
	return nil
}
...
func (br *bitcaskRange) Key(_ context.Context) []byte {
	if br.cur != nil && br.cur.Key() != nil {
		return larry.KeyFromComposite(br.table, br.cur.Key())
	}
	return nil
}
