...
var log = loggo.GetLogger("badger")
...
var (
	_ larry.Batch       = (*badgerBatch)(nil)
	_ larry.Database    = (*badgerDB)(nil)
	_ larry.Iterator    = (*badgerIterator)(nil)
	_ larry.Range       = (*badgerRange)(nil)
	_ larry.Transaction = (*badgerTX)(nil)
)
...
func NewBadgerDB(cfg *BadgerConfig) (larry.Database, error) {
	if cfg == nil {
...
func (b *badgerDB) Del(_ context.Context, table string, key []byte) error {
	...
	err := b.db.Update(func(txn *badger.Txn) error {
		return txn.Delete(larry.NewCompositeKey(table, key))
	})
	if err != nil {
...
func (b *badgerDB) Get(_ context.Context, table string, key []byte) ([]byte, error) {
	...
	err := b.db.View(func(txn *badger.Txn) error {
		item, err := txn.Get(larry.NewCompositeKey(table, key))
		if err != nil {
			return err
		}
...
func (b *badgerDB) Put(_ context.Context, table string, key, value []byte) error {
	...
	err := b.db.Update(func(txn *badger.Txn) error {
		return txn.Set(larry.NewCompositeKey(table, key), value)
	})
	if err != nil {
		return xerr(err)
	}
...
func (b *badgerDB) Begin(_ context.Context, write bool) (larry.Transaction, error) {
	if write {
		b.txMtx.Lock()
	}
...
func (b *badgerDB) execute(ctx context.Context, write bool, callback func(ctx context.Context, tx larry.Transaction) error) error {
...
func (b *badgerDB) View(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return b.execute(ctx, false, callback)
}

func (b *badgerDB) Update(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
...
func (b *badgerDB) NewIterator(ctx context.Context, table string) (larry.Iterator, error) {
	...
	opts.Prefix = larry.NewCompositeKey(table, nil)
...
func (b *badgerDB) NewBatch(ctx context.Context) (larry.Batch, error) {
...
func (tx *badgerTX) Del(ctx context.Context, table string, key []byte) error {
	...
	return xerr(tx.tx.Delete(larry.NewCompositeKey(table, key)))
}
...
func (tx *badgerTX) Get(ctx context.Context, table string, key []byte) ([]byte, error) {
	...
	item, err := tx.tx.Get(larry.NewCompositeKey(table, key))
	if err != nil {
		return nil, xerr(err)
	}
	val, err := item.ValueCopy(nil)
...
func (tx *badgerTX) Put(ctx context.Context, table string, key []byte, value []byte) error {
	...
	return xerr(tx.tx.Set(larry.NewCompositeKey(table, key), value))
}
...
func (tx *badgerTX) Write(ctx context.Context, b larry.Batch) error {
	bb, ok := b.(*badgerBatch)
	if !ok {
...
func (ni *badgerIterator) Seek(_ context.Context, key []byte) bool {
	ni.first = true
	ni.it.Seek(larry.NewCompositeKey(ni.table, key))
	return ni.it.Valid()
}

func (ni *badgerIterator) Key(_ context.Context) []byte {
	return larry.KeyFromComposite(ni.table, ni.it.Item().KeyCopy(nil))
}
...
func (nr *badgerRange) Key(ctx context.Context) []byte {
	...
	if belowLimit && valid {
		return larry.KeyFromComposite(nr.table, key)
	}
	return nil
}
