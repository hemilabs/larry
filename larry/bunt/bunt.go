...
var (
	_ larry.Batch       = (*buntBatch)(nil)
	_ larry.Database    = (*buntDB)(nil)
	_ larry.Iterator    = (*buntIterator)(nil)
	_ larry.Range       = (*buntIterator)(nil)
	_ larry.Transaction = (*buntTX)(nil)
)
...
func NewBuntDB(cfg *BuntConfig) (larry.Database, error) {
	if cfg == nil {
...
func (b *buntDB) Del(pctx context.Context, table string, key []byte) error {
	return b.Update(pctx, func(ctx context.Context, tx larry.Transaction) error {
...
func (b *buntDB) Has(pctx context.Context, table string, key []byte) (bool, error) {
	var has bool
	err := b.View(pctx, func(ctx context.Context, tx larry.Transaction) error {
...
func (b *buntDB) Begin(_ context.Context, write bool) (larry.Transaction, error) {
...
func (b *buntDB) execute(ctx context.Context, write bool, callback func(ctx context.Context, tx larry.Transaction) error) error {
...
func (b *buntDB) View(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return xerr(b.execute(ctx, false, callback))
}

func (b *buntDB) Update(ctx context.Context, callback func(ctx context.Context, tx larry.Transaction) error) error {
	return xerr(b.execute(ctx, true, callback))
}

func (b *buntDB) NewIterator(ctx context.Context, table string) (larry.Iterator, error) {
	...
	start, end := larry.BytesPrefix(larry.NewCompositeKey(table, nil))
	return &buntIterator{
		table: table,
		tx:    btx,
		start: string(start),
...
func (b *buntDB) NewRange(ctx context.Context, table string, start, end []byte) (larry.Range, error) {
	if _, ok := b.tables[table]; !ok {
		return nil, larry.ErrTableNotFound
	}
	tx, err := b.Begin(ctx, false)
...
func (b *buntDB) NewBatch(ctx context.Context) (larry.Batch, error) {
...
func (tx *buntTX) Del(ctx context.Context, table string, key []byte) error {
	...
	_, err := tx.tx.Delete(string(larry.NewCompositeKey(table, key)))
	err = xerr(err)
...
func (tx *buntTX) Get(ctx context.Context, table string, key []byte) ([]byte, error) {
	...
	val, err := tx.tx.Get(string(larry.NewCompositeKey(table, key)))
	if err != nil {
		return nil, xerr(err)
	}
	return []byte(val), nil
...
func (tx *buntTX) Put(ctx context.Context, table string, key []byte, value []byte) error {
	...
	_, _, err := tx.tx.Set(string(larry.NewCompositeKey(table, key)),
		string(value), nil)
	return xerr(err)
}
...
func (tx *buntTX) Write(ctx context.Context, b larry.Batch) error {
	bb, ok := b.(*buntBatch)
	if !ok {
...
func (bi *buntIterator) First(ctx context.Context) bool {
	pkey := larry.KeyFromComposite(bi.table, []byte(bi.start))
...
func (bi *buntIterator) Seek(_ context.Context, key []byte) bool {
	bi.first = true
	bi.cur = ""
	pkey := larry.NewCompositeKey(bi.table, key)
...
func (bi *buntIterator) Key(_ context.Context) []byte {
	return larry.KeyFromComposite(bi.table, []byte(bi.cur))
}

func (bi *buntIterator) Value(ctx context.Context) []byte {
	key := larry.KeyFromComposite(bi.table, []byte(bi.cur))
